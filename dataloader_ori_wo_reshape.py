...
class TwoScanIterator(Iterator):
    ...
    def one_hot_encode_3D(self, patch, labels):
        ...
        labels = np.array(labels)  # i.e. [0,3,4,5,6,7]
...
class TwoScanIterator(Iterator):
    ...
    def __init__(self,
                 directory, a_dir_name='ori_ct', b_dir_name='gdth_ct', c_dir_name='aux_gdth',
                 sub_dir = 'GLUCOL_isotropic1dot5',
                 a_extension='.mhd', b_extension='.nrrd', c_extension='.nrrd',
                 N=-1,
                 ptch_sz=64, ptch_z_sz=16,
    ...
    def load_scan(self, file_name, extension):
        ...
        # print ('time before isotropic:', time3)
        # # time.sleep(120)
        # zoom_seq = np.array (spacing, dtype='float') / np.array ([0.5, 0.5, 0.5], dtype='float')
        # scan = ndimage.interpolation.zoom (scan, zoom_seq, order=1, prefilter=1)
        # time4 = time.time ()
        # print ('time after isotropic:', time4)
        # print ('time during isotropic:', time4 - time3)
    ...
    def _load_img_pair(self, idx):
        ...
        a = self.load_scan(file_name=os.path.join(self.a_dir, a_fname), extension=self.a_extension) # (200, 512, 512, 1)
        b = self.load_scan(file_name=os.path.join(self.b_dir, b_fname), extension=self.b_extension) # (200, 512, 512, 1)
        if self.aux:
            c = self.load_scan(file_name=os.path.join(self.c_dir, c_fname), extension=self.c_extension) # (200, 512, 512, 1)
        ...
        if self.trgt_sz != 0 or self.trgt_space is not None:
            a = self.downscale_scan(a, order=1)
            b = self.downscale_scan(b, order=0)
    ...
    def downscale_scan(self, scan,  order=1): # scan.shape: (200, 512, 512, 1)
        ...
        time1 = time.time()
        print ('time before downscle:', time1)
        if self.trgt_space is not None:  # put trgt space more priority
            print('downsample to target space')

            zoom_seq = np.array(self.spacing, dtype='float') / np.array([self.trgt_z_space, self.trgt_space, self.trgt_space], dtype='float')   # order is correct
            zoom_seq = np.append(zoom_seq, 1)  # because here the scan is 4 dimentions

        else:
            print('downsample to target size')
            zoom_seq = np.array ([self.trgt_z_sz, self.trgt_sz, self.trgt_sz, 1], dtype='float') / np.array (scan.shape, dtype='float') # order is correct
        # zoom_seq = np.array([z_length, sz, sz, 1], dtype='float') / np.array([100, 100, 100, 1], dtype='float')
